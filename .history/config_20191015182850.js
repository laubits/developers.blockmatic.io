const config = {
	"gatsby": {
		"pathPrefix": "/",
		"siteUrl": "https://emanate.live",
		"gaTrackingId": null
	},
	"header": {
		"title": "Emanate Developers",
		"githubUrl": "https://github.com/Emanate-official/mn8-wiki",
		"helpUrl": "",
		"tweetText": "",
		"links": [
			{ "text": "", "link": ""}
		],
		"search": {
			"enabled": false,
			"indexName": "",
			"algoliaAppId": process.env.GATSBY_ALGOLIA_APP_ID,
			"algoliaSearchKey": process.env.GATSBY_ALGOLIA_SEARCH_KEY,
			"algoliaAdminKey": process.env.ALGOLIA_ADMIN_KEY
		}
	},
	"sidebar": {
		"forcedNavOrder": [
      "/tokenomics",
      "/system-overview",
      "/development-process",
      "/brand-style-guide",
      "/cloud-infrastructure",
      "/cloud-infrastructure/infra-as-code"
    ],
		"links": [
      { "text": "Github Organization", "link": "https://github.com/Emanate-Official"},
      { "text": "Emanate Whitepaper", "link": "https://emanate.live/pdf/emanate-WP-latest.pdf"}
		],
		"frontline": false,
		"ignoreIndex": true,
	},
	"siteMetadata": {
		"title": "Emanate Developers",
		"description": "",
		"ogImage": null,
		"docsLocation": "https://github.com/Emanate-official/mn8-wiki/tree/master/content",
		"favicon": "http://emanate.live/favicon.ico"
	},
};

module.exports = config;
